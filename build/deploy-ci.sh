...
mkdir temp_web
git config --global user.name "composite-bot"
...
if [ "$TRAVIS_TAG" ]; then
  ...
  cd temp_web
  git clone --depth 1 -b gh-pages --single-branch https://$ROT_TOKEN@github.com/ElemeFE/composite.git && cd composite
  # build sub folder
  echo $TRAVIS_TAG
  ...
  rm -rf $SUB_FOLDER/**
  cp -rf ../../examples/composite-ui/** .
  cp -rf ../../examples/composite-ui/** $SUB_FOLDER/
  git add -A .
...
mkdir $TRAVIS_BRANCH
rm -rf $TRAVIS_BRANCH/**
cp -rf ../../examples/composite-ui/** $TRAVIS_BRANCH/
git add -A .
git commit -m "$TRAVIS_COMMIT_MSG"
