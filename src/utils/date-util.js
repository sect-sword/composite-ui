import fecha from 'composite-ui/src/utils/date';
import { t } from 'composite-ui/src/locale';
...
export const formatDate = function(date, format) {
  ...
  if (!dateTime) return '';
  return fecha.format(dateTime, format || 'yyyy-MM-dd', getI18nSettings());
};
...
export const modifyWithTimeString = (date, time) => {
  ...
  const timeParse = parseDate(time, 'HH:mm:ss');
  return modifyTime(date, timeParse.getHours(), timeParse.getMinutes(), timeParse.getSeconds());
};
