 ...
 */
export function toggleClass(composite, className) {
  if (!composite || !className) {
    return;
  }
  let classString = composite.className;
  ...
  composite.className = classString;
}
    ...
    for (const it in obj) {
      if (isObject(obj[it])) {
        for (const key in obj[it]) {
          const composite = obj[it][key];
          arr.push(it + "." + key + "=" + composite);
        }
      } else {
        arr.push(it + "=" + obj[it]);
      }
